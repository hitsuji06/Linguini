...
def finalizar_orden():
    if not pedido_actual:
        messagebox.showwarning("Aviso", "No hay productos en la orden.")
        return

    ventana_ticket = tk.Toplevel(root)
    ventana_ticket.title("Ticket Final")
    ...
    for producto, precio in pedido_actual:
        ...
        producto_label.pack(anchor="w", padx=30)
...
finalizar_btn = tk.Button(
    ...
    fg="white",
    font=("Arial", 11, "bold"),
    command=finalizar_orden
)
finalizar_btn.pack(pady=5)
